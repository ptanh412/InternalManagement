...
import com.mnp.chat.dto.request.*;
...
public class SocketHandler {
    ...
    public void onEditGroupInfo(SocketIOClient client, SocketEditGroupInfoRequest request) {
        log.info(
                "Client editing group info: {} for conversation: {}",
                client.getSessionId(),
                request.getConversationId());
        ...
        if (webSocketSession.isPresent()) {
            try {
                ...
                if ((request.getGroupName() == null
                        ...
                        && (request.getGroupAvatar() == null
                                || request.getGroupAvatar().trim().isEmpty())) {
                    client.sendEvent("group-info-edit-error", "Either group name or group avatar must be provided");
                    return;
                }

                log.info(
                        "Processing group info edit - Name: '{}', Avatar URL: '{}'",
                        request.getGroupName(),
                        ...
                        .groupAvatar(
                                request.getGroupAvatar() != null
                                                && !request.getGroupAvatar()
                                                        .trim()
                                                        .isEmpty()
                                        ? request.getGroupAvatar().trim()
                                        : null)
                        .build();

                // Edit group info - this creates system messages and updates conversation
                ConversationResponse updatedConversation =
                        conversationService.editGroupInfo(request.getConversationId(), editRequest, userId);
                ...
                log.info(
                        "Group info edited successfully for conversation: {} - Name: '{}', Avatar: '{}'",
                        request.getConversationId(),
                        updatedConversation.getGroupName(),
    ...
    public void onForwardMessage(SocketIOClient client, ForwardMessageRequest request) {
        log.info(
                "Client forwarding message: {} to conversation: {}",
                request.getMessageId(),
        ...
        if (webSocketSession.isPresent()) {
            try {
                ...
                log.error(
                        "Error forwarding message {} to conversation {}",
                        request.getMessageId(),
                        request.getToConversationId(),
                        e);
