...
import AIMemberRecommendation from './AIMemberRecommendation';
...
const TeamLeadTaskManagement = ({ showNotification }) => {
  ...
  // AI Member Recommendation states
  ...
  const handleAIRecommendTask = (task) => {
    setSelectedTaskForAI(task);
    setAiRecommendationOpen(true);
  };

  const handleAIAssignMember = async (userId, recommendationData) => {
    try {
      if (!selectedTaskForAI) return;
      
      // Assign task using AI recommendation
      await taskService.assignTask(selectedTaskForAI.id, userId);
      
      // Update task with AI recommendation info
      const confidenceScore = recommendationData ? 
        (recommendationData.overallScore * 100).toFixed(1) : '85';
      
      await taskService.updateTask(selectedTaskForAI.id, {
        comments: `Task assigned by AI recommendation to ${userId} (Confidence: ${confidenceScore}%)`
      });
      
      setAiRecommendationOpen(false);
      setSelectedTaskForAI(null);
      showNotification(`Task assigned to ${userId} using AI recommendation!`, 'success');
      await loadTasks(); // Reload tasks to get updated data
    } catch (error) {
      console.error('Failed to assign task with AI recommendation:', error);
  ...
  return (
    <Container maxWidth="xl">
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            Review and manage team task assignments and approvals
          </Typography>
        </Box>
                ...
                {task.status === 'pending_approval' && (
                  <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
                    ...
                    <Button
                      ...
                      color="info"
                      startIcon={<AIIcon />}
                      onClick={() => handleAIRecommendTask(task)}
                      sx={{
                        background: `linear-gradient(45deg, ${theme.palette.info.main}20, ${theme.palette.primary.main}20)`,
                      }}
                    >
                      AI Assign
                    </Button>
      ...
      {/* AI Member Recommendation Dialog */}
      <AIMemberRecommendation
        open={aiRecommendationOpen}
        onClose={() => {
          setAiRecommendationOpen(false);
          setSelectedTaskForAI(null);
        }}
        taskId={selectedTaskForAI?.id}
        taskTitle={selectedTaskForAI?.title}
        onMemberSelect={handleAIAssignMember}
        showNotification={showNotification}
      />
    </Container>
  );
