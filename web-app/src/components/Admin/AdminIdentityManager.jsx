...
const AdminIdentityManager = ({ showNotification }) => {
  ...
  const handleCVParse = async () => {
    ...
    try {
      ...
      if (response.result && response.result.processingStatus === 'SUCCESS') {
        ...
        setCvDataPreviewOpen(true);
        showNotification('CV parsed successfully! Review the extracted data.', 'success');
      } else {
  ...
  const handleClearCVData = () => {
    ...
    setCvDataPreviewOpen(false);
    setCvFieldMappingOpen(false);
    setCvMappedData(null);
  ...
  const handleAcceptCVData = () => {
    setCvDataPreviewOpen(false);
    setCvFieldMappingOpen(true);
  };

  const handleRejectCVData = () => {
    setCvDataPreviewOpen(false);
    showNotification('CV data rejected. You can upload a different CV or create user manually.', 'info');
  };

  const handleCVMappingComplete = (mappedData) => {
    setCvFieldMappingOpen(false);
    setCvMappedData(mappedData);
    ...
    if (mappedData.userData) {
      setUserFormData(prev => ({
        ...prev,
        ...mappedData.userData,
        username: mappedData.userData.email?.split('@')[0] || prev.username,
        password: '', // Always require manual password input
        departmentId: mappedData.suggestions.department || prev.departmentId,
        positionId: mappedData.suggestions.position || prev.positionId,
        roles: mappedData.suggestions.roles || prev.roles
      }));
    }
    
    setShowCvPreview(true);
    showNotification('CV data mapped successfully! Form has been auto-filled.', 'success');
  };
  ...
  return (
    <Box>
      ...
      <CVDataPreview
        ...
        open={cvDataPreviewOpen}
        onClose={() => setCvDataPreviewOpen(false)}
        onAcceptData={handleAcceptCVData}
        onRejectData={handleRejectCVData}
        aiService={aiService}
      />

      {/* CV Field Mapping Dialog */}
      <CVFieldMappingDialog
        open={cvFieldMappingOpen}
        onClose={() => setCvFieldMappingOpen(false)}
        cvData={cvParsingResult}
        formData={userFormData}
        onMappingComplete={handleCVMappingComplete}
